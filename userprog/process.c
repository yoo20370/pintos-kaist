...
tid_t process_create_initd(const char *file_name)
{
	...
	char *fn_copy_for_token = palloc_get_page(0);
	if (fn_copy_for_token == NULL)
		return TID_ERROR;
	strlcpy(fn_copy_for_token, fn_copy, PGSIZE);

	token = strtok_r(fn_copy_for_token, " ", &saveptr1);
